...
export async function authenticatedFetchServer(
  ...
  try {
    ...
    if (fetchError instanceof Response && fetchError.status >= 300 && fetchError.status < 400) {
...
async function fetchJsonServer(url: string, options: RequestInit = {}, event: RequestEvent) {
...
export async function getClientsServer(event: RequestEvent): Promise<Response> {
  return await authenticatedFetchServer(
...
export const createStaffProfileServer = (data: any, event: RequestEvent) =>
  fetchJsonServer(`${API_BASE_URL}/staff-profiles`, { 
    method: 'POST', 
    body: JSON.stringify(data) 
  }, event);

export const updateStaffProfileServer = (id: string, data: any, event: RequestEvent) =>
  fetchJsonServer(`${API_BASE_URL}/staff-profiles/${id}`, { 
    method: 'PUT', 
    body: JSON.stringify(data) 
  }, event);
...
export const createClientServer = (data: any, event: RequestEvent) =>
  fetchJsonServer(`${API_BASE_URL}/clients`, { 
    method: 'POST', 
    body: JSON.stringify(data) 
  }, event);

export const updateClientServer = (id: string, data: any, event: RequestEvent) =>
  fetchJsonServer(`${API_BASE_URL}/clients/${id}`, { 
    method: 'PUT', 
    body: JSON.stringify(data) 
  }, event);
