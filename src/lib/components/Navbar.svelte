  ...
  const roleBasedItems = $derived(() => {
    ...
    items.push(
      { label: "Profile", href: "/profile", icon: "User", badge: null },
      {
        ...
        icon: "Calendar",
        badge: null,
        roles: ["Super Admin", "Admin", "Dispatcher", "Driver"],
      },
...
<Sidebar.Provider>
  <Sidebar.Root class="w-64 bg-slate-50 border-r border-slate-200">
    ...
    <Sidebar.Content class="p-4">
      ...
      <Sidebar.Group>
        <Sidebar.GroupLabel
          class="text-slate-600 font-medium text-sm uppercase tracking-wide mb-3"
      ...
      {#if quickActions().length > 0}
        <Sidebar.Group>
          <Sidebar.GroupLabel
            class="text-slate-600 font-medium text-sm uppercase tracking-wide mb-3"
            >Quick Actions</Sidebar.GroupLabel
          >
          <Sidebar.Menu>
