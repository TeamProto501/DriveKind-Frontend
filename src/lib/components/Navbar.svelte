...
<script lang="ts">
	...
	import { getNavigationItems, getQuickActions, getDefaultRoute } from '$lib/navigation';
	...
	const userInitials = $derived(userProfile 
		? `${userProfile.first_name?.[0] || ''}${userProfile.last_name?.[0] || ''}`.toUpperCase()
		: session?.user?.email?.[0]?.toUpperCase() || 'U');
	
	// Get display name
	const displayName = $derived(userProfile 
...
<nav class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-50">
		...
		<div class="flex justify-between items-center h-16">
			...
			{#if session && !isLoading}
				<div class="hidden md:flex items-center space-x-1">
					{#each mainNavItems as item}
						...
						<a
							href={item.href}
							class="px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200 flex items-center space-x-2 {isActiveRoute(item.href) 
					...
					{#if moreNavItems.length > 0}
						<div class="relative more-dropdown">
							...
							{#if isMoreDropdownOpen}
								<div class="absolute right-0 mt-2 w-64 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 divide-y divide-gray-100">
									<div class="px-4 py-3">
			...
			<div class="flex items-center space-x-2">
				{#if session && !isLoading}
					...
					{#if quickActions.length > 0}
						<div class="relative quick-actions">
							...
							{#if isQuickActionsOpen}
								<div class="absolute right-0 mt-2 w-56 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 divide-y divide-gray-100">
									<div class="px-3 py-2">
					...
					<!-- Notifications -->
					<button class="p-1.5 rounded text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors duration-200 relative">
						<Bell class="w-4 h-4" />
						<!-- Notification badge -->
	...
	{#if session && !isLoading && isMobileMenuOpen}
	...
	<div class="bg-green-600 text-white">
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
			<div class="flex items-center justify-center space-x-3">
				...
				<a href="tel:+1-555-DRIVE-KIND" class="text-xl font-bold hover:underline">
					(555) DRIVE-KIND
				</a>
			</div>
		</div>
