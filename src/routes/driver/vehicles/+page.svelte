<script lang="ts">
  ...
  async function createVehicle() {
    if (!uid) {
      setToast("No user session.", false);
      return;
    }
  ...
  async function saveEdits() {
    ...
    try {
      ...
      const res = await fetch(
        `/driver/vehicles/update/${editForm.vehicle_id}`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
  ...
  <div class="min-h-screen bg-gray-50">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600">
                View your vehicles and choose the active one.
              </p>
            </div>
          </div>
          <button
    ...
    {#if toast}
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
        ...
        >
          <p
            class={toastOk ? "text-sm text-green-800" : "text-sm text-red-800"}
          >
            {toast}
          </p>
        </div>
      ...
      {:else if vehicles.length === 0}
        ...
        >
          ...
          <p class="text-gray-500">
            Your account doesn’t have any vehicles yet.
          </p>
        </div>
      {:else}
        <div class="space-y-6">
          {#each vehicles as v, index (v.vehicle_id)}
            <div
