...
<svelte:head>
  <title>Driver Schedules &amp; Unavailability - DriveKind</title>
</svelte:head>
...
<div class="container mx-auto py-6 space-y-6">
  ...
  <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
    <div>
      <h1 class="text-3xl font-bold tracking-tight">
        Driver Unavailability &amp; Time Off
      </h1>
      <p class="text-muted-foreground mt-1">
        Select a driver to view and manage their unavailable times
      </p>
    </div>

    {#if data.drivers && data.drivers.length > 0}
      <form method="GET" class="w-full md:w-72 space-y-1">
        ...
        >
          {#each data.drivers as driver}
            <option
              ...
              selected={driver.user_id === data.selectedUserId}
            >
              {driver.full_name}
            </option>
          {/each}
      ...
      <Card.Content class="pt-6">
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1">
            ...
            <p class="text-sm text-red-800 mt-1">{data.error}</p>
          </div>
        </div>
      </Card.Content>
    </Card.Root>
  ...
  {#if !data.drivers || data.drivers.length === 0}
    <Card.Root>
      <Card.Header>
        <Card.Title>No drivers found</Card.Title>
        <Card.Description>
          There are no staff in your organization with the "Driver" role.
        </Card.Description>
      </Card.Header>
    </Card.Root>
  {:else if !data.selectedUserId}
    <Card.Root>
      <Card.Header>
        <Card.Title>Select a driver</Card.Title>
        <Card.Description>
          Choose a driver from the dropdown above to view their schedule.
        </Card.Description>
      </Card.Header>
    </Card.Root>
  {:else}
    ...
    <Card.Root>
      <Card.Header>
        <Card.Title>Driver Unavailability Schedule</Card.Title>
        <Card.Description>
          View and manage all unavailable times for this driver
        </Card.Description>
      </Card.Header>
      <Card.Content>
        <UnavailabilityTable
          data={data.unavailability || []}
      ...
      <Tabs.Content value="one-time">
        <Card.Root>
          <Card.Header>
            ...
            <Card.Description>
              Set this driver as unavailable for a specific date
            </Card.Description>
          </Card.Header>
          <Card.Content>
            <form
            ...
            >
              ...
              <div class="space-y-2">
                ...
                <Select.Root
                  ...
                  name="reason"
                  value={oneTimeReason}
                  onValueChange={(v) => (oneTimeReason = v)}
                >
                  <Select.Trigger id="one-reason">
                    <span class="truncate">
                      {ONE_TIME_REASONS.find((r) => r.value === oneTimeReason)
                        ?.label || "Select a reason"}
                    </span>
                  </Select.Trigger>
                  <Select.Content>
                    {#each ONE_TIME_REASONS as reason}
                      <Select.Item value={reason.value}>
                        {reason.label}
                      </Select.Item>
                    {/each}
                  </Select.Content>
      ...
      <Tabs.Content value="range">
        <Card.Root>
          <Card.Header>
            ...
            <Card.Description>
              Set this driver as unavailable for multiple consecutive days
              (e.g., vacation)
            </Card.Description>
          </Card.Header>
          <Card.Content>
            ...
            >
              ...
              <div class="space-y-2">
                ...
                <Select.Root
                  ...
                  name="reason"
                  value={rangeReason}
                  onValueChange={(v) => (rangeReason = v)}
                >
                  <Select.Trigger id="range-reason">
                    <span class="truncate">
                      {RANGE_REASONS.find((r) => r.value === rangeReason)
                        ?.label || "Select a reason"}
                    </span>
                  </Select.Trigger>
                  <Select.Content>
      ...
      <Tabs.Content value="weekly">
        <Card.Root>
          <Card.Header>
            ...
            <Card.Description>
              Set regular weekly patterns for this driver (e.g., every Monday
              and Wednesday)
            </Card.Description>
          </Card.Header>
          <Card.Content>
            ...
            >
                ...
                <div class="flex flex-wrap gap-2">
                  {#each WEEK_DAYS as day}
                    <button
                      ...
                      class="px-4 py-2 rounded-lg border-2 transition-all {weeklyDays.includes(
                        day.value
                      )
                ...
                {:else}
                  <p class="text-sm text-blue-600 font-medium">
                    ...
                    {weeklyDays
                      .map(
                        (d) =>
                          WEEK_DAYS.find((wd) => wd.value === d)?.fullName
                      )
                      .join(", ")}
                  </p>
                {/if}
              ...
              <div class="space-y-2">
                ...
                <Select.Root
                  ...
                  name="reason"
                  value={weeklyReason}
                  onValueChange={(v) => (weeklyReason = v)}
                >
                  <Select.Trigger id="weekly-reason">
                    <span class="truncate">
                      {WEEKLY_REASONS.find((r) => r.value === weeklyReason)
                        ?.label || "Select a reason"}
                    </span>
                  </Select.Trigger>
                  <Select.Content>
