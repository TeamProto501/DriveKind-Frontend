<script lang="ts">
  ...
  let { data, selectedUserId }: Props = $props();
  ...
  {#if sortedItems.length === 0}
    <div class="text-center py-12 border rounded-lg bg-muted/50">
      ...
      <p class="text-sm text-muted-foreground">
        Add their first unavailable time using the tabs above
      </p>
    </div>
  {:else}
          ...
          {#each sortedItems as item}
            <Table.Row>
              ...
              <Table.Cell>
                <Badge
                  variant="outline"
                  class={getTypeBadgeClass(item.unavailability_type)}
                >
    ...
    <div class="text-sm text-muted-foreground">
      Showing {sortedItems.length} unavailability
      {sortedItems.length === 1 ? "entry" : "entries"}
    </div>
  {/if}
</div>