// src/routes/dispatcher/driver-schedules/+page.server.ts
...
export const load: PageServerLoad = async (event) => {
  ...
  // All drivers in this org (roles contains "Driver")
  ...
  if (driversError) {
    ...
    return {
      ...
      selectedUserId: null,
      unavailability: [],
      error: "Failed to load drivers for your organization"
    };
  }

  // Map to the shape the Svelte page expects: user_id + full_name + roles
  const drivers =
    driversRaw?.map((d) => ({
      user_id: d.user_id,
  ...
  // Determine which driver is selected (query param ?driver_id=...)
  const url = event.url;
  ...
  // Ensure selectedUserId is actually one of the drivers we loaded
  if (
    selectedUserId &&
...
export const actions: Actions = {
  createSpecificUnavailability: async (event) => {
    console.log("=== DISPATCHER: CREATE SPECIFIC UNAVAILABILITY ===");
    try {
      const supabase = createSupabaseServerClient(event);
      const {
        data: { user },
      ...
      if (!driverUserId) {
        return fail(400, { error: "Please select a driver" });
      }
  ...
  createRangeUnavailability: async (event) => {
    console.log("=== DISPATCHER: CREATE RANGE UNAVAILABILITY ===");
    try {
      const supabase = createSupabaseServerClient(event);
      const {
        data: { user },
  ...
  createRegularUnavailability: async (event) => {
    console.log("=== DISPATCHER: CREATE WEEKLY UNAVAILABILITY ===");
    try {
      const supabase = createSupabaseServerClient(event);
      const {
        data: { user },
  ...
  deleteUnavailability: async (event) => {
    console.log("=== DISPATCHER: DELETE UNAVAILABILITY ===");
    try {
      const supabase = createSupabaseServerClient(event);
      const {
        data: { user },
      ...
      if (!id || !driverUserId) {
        return fail(400, { error: "Invalid unavailability or driver ID" });
      }
