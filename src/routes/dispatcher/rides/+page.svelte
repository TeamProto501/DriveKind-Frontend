<script lang="ts">
  ...
  async function forceAcceptRide(driverId: string) {
    ...
    console.log('Force accepting ride:', selectedRide.ride_id, 'for driver:', driverId);
    
    isUpdating = true;
    try {
      const response = await fetch(`/dispatcher/rides/force-accept/${selectedRide.ride_id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      alert('Error force accepting ride. Check console for details.');
    } finally {
      isUpdating = false;
    }
  }
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      <div class="mb-4">
        ...
        <p class="text-sm text-gray-600">Select a driver to force accept this ride. This will bypass the driver's acceptance and immediately schedule the ride.</p>
      </div>
        ...
        {#each filteredDrivers() as driver}
          <div class="flex items-center justify-between p-3 border rounded-lg hover:bg-gray-50">
            ...
            <button 
              ...
              disabled={isUpdating}
              class="px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-sm transition-colors disabled:opacity-50"
            >
              Force Accept
            </button>
          </div>
      ...
      <div class="flex justify-end">
        <button 
          onclick={() => showForceAcceptModal = false}
          class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors"
        >
          Cancel
        </button>
