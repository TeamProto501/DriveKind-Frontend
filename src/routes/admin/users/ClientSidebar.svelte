<script lang="ts">
  ...
  type Client = {
    ...
    residence_enum: 'house'|'apartment'|'mobile home'|'townhouse'|'condo';
  ...
  const residenceOptions = ['house', 'apartment', 'mobile home', 'townhouse', 'condo'] as const;
  ...
  function initForm(): Client {
    if (!client || createMode) {
      return {
        first_name: '', last_name: '', email: null, primary_phone: '', secondary_phone: null,
        contact_pref: null, date_of_birth: '', gender: 'Other', street_address: '', address2: null,
        city: '', state: '', zip_code: '', lives_alone: false,
        primaryPhone_is_cell: true, primaryPhone_can_text: true,
        secondaryPhone_is_cell: false, secondaryPhone_can_text: false,
        service_animal: false, oxygen: false,
        client_status_enum: 'Active',
        mobility_assistance_enum: null, residence_enum: 'house',
        ...
        allergies: null, other_allergies: null, pick_up_instructions: null, comments: null, other_limitations: null,
        referral_method: null, driver_preference: null,
        org_id: orgId
      };
    }
  ...
  async function saveClient() {
    const e=[...errsStep(1),...errsStep(2)]; if(e.length){ errorMessage=e.join(' '); return; }
    saving = true;
    try {
      const body = {
        ...form,
      ...
      if (createMode) {
        ...
        if(!res.ok){ throw new Error(await res.text() || 'Failed to create client'); }
        ...
        if(!res.ok){ throw new Error(await res.text() || 'Failed to update client'); }
...
<div class="fixed top-0 right-0 w-[28rem] max-w-[92vw] h-full bg-white shadow-xl border-l border-gray-200 flex flex-col z-50">
  ...
  <div class="px-6 py-4 border-b flex items-center justify-between">
    <h2 class="text-lg md:text-xl font-semibold text-gray-900">
      {createMode ? 'Add New Client' : mode==='view' ? 'Client Profile' : 'Edit Client'}
    </h2>
    ...
    {#if !createMode && mode==='view' && client}
      ...
      <div class="space-y-4 text-[15px] leading-6">
        <div class="bg-gray-50 rounded-lg p-3">
          <div class="font-semibold text-gray-900 text-lg">{client.first_name} {client.last_name}</div>
          <div class="text-gray-600">{client.gender} • {client.client_status_enum}</div>
        </div>

        <div class="grid grid-cols-1 gap-3">
          <div class="grid grid-cols-2 gap-3">
            <div><div class="text-xs text-gray-500">Primary Phone</div><div class="font-medium">{client.primary_phone}</div></div>
            <div><div class="text-xs text-gray-500">Secondary Phone</div><div class="font-medium">{client.secondary_phone || '—'}</div></div>
          </div>
          <div><div class="text-xs text-gray-500">Email</div><div class="font-medium">{client.email || '—'}</div></div>
          <div><div class="text-xs text-gray-500">Address</div>
            <div class="font-medium">{[client.street_address, client.address2].filter(Boolean).join(' ')}</div>
            <div class="font-medium">{client.city}, {client.state} {client.zip_code}</div>
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div><div class="text-xs text-gray-500">DOB</div><div class="font-medium">{client.date_of_birth}</div></div>
            <div><div class="text-xs text-gray-500">Contact Preference</div><div class="font-medium">{client.contact_pref || '—'}</div></div>
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div><div class="text-xs text-gray-500">Lives Alone</div><div class="font-medium">{client.lives_alone ? 'Yes' : 'No'}</div></div>
            <div><div class="text-xs text-gray-500">Service Animal</div><div class="font-medium">{client.service_animal ? 'Yes' : 'No'}</div></div>
          </div>

          <div class="grid grid-cols-2 gap-3">
            <div><div class="text-xs text-gray-500">Mobility Assistance</div><div class="font-medium">{client.mobility_assistance_enum || '—'}</div></div>
            <div><div class="text-xs text-gray-500">Residence</div><div class="font-medium">{client.residence_enum}</div></div>
          </div>

          <div class="grid grid-cols-1 gap-2">
            <div><div class="text-xs text-gray-500">Emergency Contact</div><div class="font-medium">{client.emergency_contact_name || '—'}{client.emergency_contact_relationship ? ` (${client.emergency_contact_relationship})` : ''}</div></div>
            <div><div class="text-xs text-gray-500">Emergency Phone</div><div class="font-medium">{client.emergency_contact_phone || '—'}</div></div>
          </div>

          <div class="grid grid-cols-1 gap-2">
            <div><div class="text-xs text-gray-500">Allergies</div><div class="font-medium">{client.allergies || client.other_allergies || '—'}</div></div>
            <div><div class="text-xs text-gray-500">Pickup Instructions</div><div class="font-medium">{client.pick_up_instructions || '—'}</div></div>
            <div><div class="text-xs text-gray-500">Comments</div><div class="font-medium">{client.comments || '—'}</div></div>
            <div><div class="text-xs text-gray-500">Other Limitations</div><div class="font-medium">{client.other_limitations || '—'}</div></div>
          </div>
        </div>
      </div>
    ...
    {:else}
      ...
      <div class="flex items-center justify-center gap-3 mb-2">
        {#each [1,2,3,4] as s}
          <div class="flex items-center gap-2">
            ...
            {#if s<4}<div class="w-8 h-[2px] {step>s ? 'bg-blue-600' : 'bg-gray-200'}"></div>{/if}
          </div>
        {/each}
      </div>
      ...
      {#if step===2}
        ...
        <div class="space-y-3">
          ...
          <div class="grid grid-cols-2 gap-3">
            <label class="flex items-center gap-2 text-base"><input type="checkbox" bind:checked={form.primaryPhone_can_text} /> Primary can text</label>
            <label class="flex items-center gap-2 text-base"><input type="checkbox" bind:checked={form.secondaryPhone_is_cell} /> Secondary phone is cell</label>
          </div>
          <div class="grid grid-cols-2 gap-3">
            <label class="flex items-center gap-2 text-base"><input type="checkbox" bind:checked={form.secondaryPhone_can_text} /> Secondary can text</label>
            <span></span>
          </div>
        </div>
      {/if}
      ...
      {#if step===3}
        ...
        <div class="space-y-3">
          <div class="grid grid-cols-2 gap-3">
            <div><label class="block text-base font-medium">Mobility Assistance</label>
              <select class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.mobility_assistance_enum}>
                {#each mobilityOptions as m}<option value={m}>{m || 'None'}</option>{/each}
              </select>
            </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <label class="flex items-center gap-2 text-base"><input type="checkbox" bind:checked={form.service_animal} /> Service animal</label>
            <label class="flex items-center gap-2 text-base"><input type="checkbox" bind:checked={form.oxygen} /> Requires oxygen</label>
          </div>
          <div><label class="block text-base font-medium">Service Animal Size</label>
            <select class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.service_animal_size_enum}>
              {#each serviceAnimalSizeOptions as s}<option value={s}>{s || 'None'}</option>{/each}
            </select>
          </div>

          <div class="grid grid-cols-1 gap-3">
            <div><label class="block text-base font-medium">Allergies</label><textarea rows="2" class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.allergies} /></div>
            <div><label class="block text-base font-medium">Other Allergies</label><textarea rows="2" class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.other_allergies} /></div>
            <div><label class="block text-base font-medium">Other Limitations</label><textarea rows="2" class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.other_limitations} /></div>
          </div>
        </div>
      {/if}
      ...
      {#if step===4}
        ...
        <div class="space-y-3">
          ...
          {#if form.client_status_enum==='Temporary Thru'}
            <div><label class="block text-base font-medium">Temporary Client Date</label>
              <input type="date" class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.temp_client_date} />
            </div>
          {/if}
          <div class="grid grid-cols-2 gap-3">
            <div><label class="block text-base font-medium">Referral Method</label><input class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.referral_method} /></div>
            <div><label class="block text-base font-medium">Driver Preference</label><input class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.driver_preference} /></div>
          </div>
          <div class="grid grid-cols-1 gap-3">
            <div><label class="block text-base font-medium">Pickup Instructions</label><textarea rows="2" class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.pick_up_instructions} /></div>
            <div><label class="block text-base font-medium">Comments</label><textarea rows="3" class="mt-1 w-full border rounded px-3 py-2 text-base" bind:value={form.comments} /></div>
          </div>
        </div>
      {/if}
    {/if}
  ...
  {#if createMode || mode==='edit'}
    <div class="px-6 py-4 border-t flex items-center justify-between">
      <div>{#if step>1}<button on:click={back} class="px-4 py-2 rounded-lg border">Back</button>{/if}</div>
      <div class="flex gap-2">
        ...
        {:else}
          <button on:click={saveClient} disabled={saving} class="px-4 py-2 rounded-lg bg-blue-600 text-white disabled:opacity-50">
