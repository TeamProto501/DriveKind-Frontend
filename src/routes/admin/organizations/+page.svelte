<script lang="ts">
	...
	const HOUR_OPTS = Array.from({length:24}, (_,i) => String(i).padStart(2,'0')); // '00'..'23'
	...
	function validateRequiredForStep(stepIdx: number): string[] {
		...
		const requiredByStep: Record<number, string[]> = {
			...
			2: ['rides_phone_number','client_min_age','min_days_in_advance_for_ride_requests'], // working hours via UI
	...
	function applyFilter() {
		...
		filteredOrganizations = filteredOrganizations.slice().sort((a,b) => (a.org_id ?? 0) - (b.org_id ?? 0));
	}
	...
	function openEditModal(org: OrgRow) {
		...
		whEdit = parseWorkingHoursToUI(org.working_hours ?? org['working_hours'] ?? '');
	...
	{#if editMessage}
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
			<div class="rounded-md p-4 {editMessageSuccess ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}">
				<p class="text-sm font-medium {editMessageSuccess ? 'text-green-800' : 'text-red-800'}">{editMessage}</p>
			</div>
		</div>
	{/if}
			...
			<div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
				<div class="mt-3">
					...
					<div class="mb-6">
						...
						<input type="password" bind:value={passwordInput} class="w-full px-3 py-2 border {passwordError ? 'border-red-300' : 'border-gray-300'} rounded-md" />
						{#if passwordError}<p class="mt-2 text-sm text-red-600">{passwordError}</p>{/if}
					</div>
					<div class="flex justify-end gap-3">
						<button onclick={closeModals} class="px-4 py-2 border border-gray-300 rounded-md">Cancel</button>
						<button onclick={confirmDeleteWithPassword} class="px-4 py-2 bg-red-600 text-white rounded-md">Delete Organization</button>
					</div>
				</div>
			</div>
		</div>
				...
				<form onsubmit={addOrganization} class="space-y-6">
					...
					{#if addStep === 0}
						<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
							...
							<div><label class="block text-sm font-medium">{labelWithRequired('Website','org_website')}</label><input type="text" bind:value={form.org_website} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.org_website ? 'border-red-500' : 'border-gray-300')} /></div>
										...
										<tbody>
											{#each DAYS as d}
												<tr class="border-t">
													...
													<td class="px-3 py-2">
														<select class="border rounded px-2 py-1" bind:value={whAdd[d.code].start} disabled={!whAdd[d.code].open}>
															{#each HOUR_OPTS as h}<option value={h}>{h}:00</option>{/each}
														</select>
													</td>
													<td class="px-3 py-2">
														<select class="border rounded px-2 py-1" bind:value={whAdd[d.code].end} disabled={!whAdd[d.code].open}>
															{#each HOUR_OPTS as h}<option value={h}>{h}:00</option>{/each}
														</select>
													</td>
												</tr>
					...
					<div class="flex justify-between pt-2">
						<button type="button" onclick={addStep === 0 ? closeModals : prevAddStep} class="px-4 py-2 border border-gray-300 rounded-md">
		...
		<div class="fixed inset-0 bg-gray-600/50 overflow-y-auto h-full w-full z-50">
			<div class="relative top-10 mx-auto p-5 w-full max-w-4xl shadow-lg rounded-2xl bg-white">
				...
				<form onsubmit={updateOrganization} class="space-y-6">
					...
					<div class="flex justify-between pt-2">
						<button type="button" onclick={editStep === 0 ? closeModals : prevEditStep} class="px-4 py-2 border border-gray-300 rounded-md">
