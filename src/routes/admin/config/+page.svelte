<script lang="ts">
	...
	$effect(() => {
		...
		const rawStatus = (org && (org.org_status ?? org.org_status_enum ?? org.status)) ?? '';
		const s = String(rawStatus).trim().toLowerCase();
		statusLabel = s === 'active' ? 'Active' : s === 'inactive' ? 'Inactive' : 'Inactive';
		statusPillClasses = statusLabel === 'Active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';
	});
	...
	const DAYS: Array<{code: DayCode; label: string}> = [
		{ code: 'Su', label: 'Sunday' }, { code: 'Mo', label: 'Monday' }, { code: 'Tu', label: 'Tuesday' },
		{ code: 'We', label: 'Wednesday' }, { code: 'Th', label: 'Thursday' }, { code: 'Fr', label: 'Friday' },
	...
	function parseWorkingHoursToUI(input?: string | null): WorkingHoursUI {
		...
		for (const token of String(input).split(',').map(s => s.trim()).filter(Boolean)) {
			...
			if (!m) continue;
			const [, code, start, end] = m as unknown as [string, DayCode, string, string];
	...
	const OPTIONAL_KEYS = new Set<string>([
		'days_off', 'org_address2', 'first_ride_date', 'last_activity_in_portal', 'primary_contact_address2',
		'secondary_contact_name','secondary_contact_email','secondary_contact_address','secondary_contact_address2',
	...
	function validateRequiredForStep(stepIdx: number): string[] {
		...
		for (const k of keys) {
			...
			if (empty) { missing.push(k); fieldErrors[k] = 'Required'; }
		}
		// Ensure at least one open day in step 2
		if (stepIdx === 2) {
			const anyOpen = DAYS.some(d => whEdit[d.code].open);
			if (!anyOpen) { missing.push('working_hours'); fieldErrors['working_hours'] = 'Select at least one open day.'; }
		}
		return missing;
	}
	...
	function openEdit() {
		...
		// normalize status
		const rawStatus = ((org as any).org_status ?? (org as any).org_status_enum ?? (org as any).status ?? '') as string;
		const s = String(rawStatus).trim().toLowerCase();
	...
	function coerceNumbers(payload: Record<string, any>, keys: string[]) {
		for (const k of keys) {
			if (payload[k] == null || payload[k] === '') { payload[k] = null; continue; }
	...
	function prevStep() { if (editStep > 0) editStep -= 1; }
	...
	async function saveOrg(e: Event) {
		...
		try {
			...
			const { error } = await supabase.from('organization').update(payload).eq('org_id', orgId);
			if (error) throw error;
...
<RoleGuard requiredRoles={['Admin', 'Super Admin']}>
	<div class="min-h-screen bg-gray-50">
		...
		<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
			...
			<div class="mb-8 flex justify-between items-center">
				...
				{#if org}
					<button on:click={openEdit} class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
			...
			{:else if org}
				<div class="space-y-6">
						...
						<div class="p-6">
							<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
								...
								<div>
									...
									{#if org.org_website}
										<a href={/^https?:\/\//.test(org.org_website) ? org.org_website : `https://${org.org_website}`} class="mt-1 text-blue-600 hover:underline flex items-center" target="_blank" rel="noopener noreferrer">
											<LinkIcon class="w-4 h-4 mr-1 text-gray-400" /> {org.org_website}
										</a>
									{:else}
								...
								<div>
									...
									<p class="mt-1 text-gray-900 flex items-center"><Mail class="w-4 h-4 mr-1 text-gray-400" /> {org.org_email ?? '-'}</p>
								</div>
								<div>
									<label class="block text-sm font-medium text-gray-700">Phone</label>
									<p class="mt-1 text-gray-900 flex items-center"><Phone class="w-4 h-4 mr-1 text-gray-400" /> {org.org_phone ?? '-'}</p>
								</div>
								<div>
						...
						<div class="p-6">
							<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div>
									...
									{#if org.org_address2}<p class="mt-1 text-gray-900">{org.org_address2}</p>{/if}
								</div>
								<div>
									<label class="block text-sm font-medium text-gray-700">City / State / Zip</label>
									<p class="mt-1 text-gray-900">{org.org_city ?? '-'}, {org.org_state ?? '-'} {org.org_zip_code ?? ''}</p>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="bg-white rounded-lg shadow-sm border border-gray-200">
						...
						<div class="p-6">
							<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div><label class="block text-sm font-medium text-gray-700">Name</label><p class="mt-1 text-gray-900">{org.primary_contact_name ?? '-'}</p></div>
								<div><label class="block text-sm font-medium text-gray-700">Email</label><p class="mt-1 text-gray-900">{org.primary_contact_email ?? '-'}</p></div>
								<div class="md:col-span-2">
									<label class="block text-sm font-medium text-gray-700">Address</label>
									<p class="mt-1 text-gray-900">{org.primary_contact_address ?? '-'}{org.primary_contact_address2 ? `, ${org.primary_contact_address2}` : ''}</p>
									<p class="mt-1 text-gray-900">{org.primary_contact_city ?? ''}{org.primary_contact_city ? ', ' : ''}{org.primary_contact_state ?? ''} {org.primary_contact_zipcode ?? ''}</p>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="bg-white rounded-lg shadow-sm border border-gray-200">
						...
						<div class="p-6">
							<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div><label class="block text-sm font-medium text-gray-700">Name</label><p class="mt-1 text-gray-900">{org.secondary_contact_name ?? '-'}</p></div>
								<div><label class="block text-sm font-medium text-gray-700">Email</label><p class="mt-1 text-gray-900">{org.secondary_contact_email ?? '-'}</p></div>
								<div class="md:col-span-2">
									<label class="block text-sm font-medium text-gray-700">Address</label>
									<p class="mt-1 text-gray-900">{org.secondary_contact_address ?? '-'}{org.secondary_contact_address2 ? `, ${org.secondary_contact_address2}` : ''}</p>
									<p class="mt-1 text-gray-900">{org.secondary_contact_city ?? ''}{org.secondary_contact_city ? ', ' : ''}{org.secondary_contact_state ?? ''} {org.secondary_contact_zipcode ?? ''}</p>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="bg-white rounded-lg shadow-sm border border-gray-200">
						...
						<div class="p-6">
							<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
								<div><label class="block text-sm font-medium text-gray-700">Created</label><p class="mt-1 text-gray-900">{org.org_creation_date ?? '-'}</p></div>
								<div><label class="block text-sm font-medium text-gray-700">First Ride Date</label><p class="mt-1 text-gray-900">{org.first_ride_date ?? '-'}</p></div>
								<div><label class="block text-sm font-medium text-gray-700">Last Activity</label><p class="mt-1 text-gray-900">{org.last_activity_in_portal ?? '-'}</p></div>
							</div>
						</div>
					</div>
				</div>
	...
	{#if showEditModal}
		<div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
			<div class="relative top-12 mx-auto p-5 border w-full max-w-4xl shadow-lg rounded-2xl bg-white">
				<div class="flex items-center justify-between mb-2">
					<h3 class="text-xl font-semibold">Edit Organization</h3>
					<button on:click={closeEdit} class="text-gray-400 hover:text-gray-600"><X class="w-5 h-5" /></button>
				</div>

				<p class="text-sm text-gray-500 mb-4"><span class="text-red-600">*</span> indicates required fields.</p>
				<div class="mb-3 text-sm text-gray-700">Step {editStep + 1} of {STEP_LABELS.length}: <span class="font-medium">{STEP_LABELS[editStep]}</span></div>
				...
				<form on:submit={saveOrg} class="space-y-6">
					...
					{#if editStep === 0}
						<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
							<div>
								...
								<input type="text" bind:value={form.name} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.name ? 'border-red-500' : 'border-gray-300')} />
							</div>
							<div>
								<label class="block text-sm font-medium">{labelWithRequired('Organization Status','org_status')}</label>
								<select bind:value={form.org_status} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.org_status ? 'border-red-500' : 'border-gray-300')}>
							...
							<div>
								...
								<input type="text" bind:value={form.org_website} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.org_website ? 'border-red-500' : 'border-gray-300')} />
							</div>
							<div>
								<label class="block text-sm font-medium">{labelWithRequired('Organization Email','org_email')}</label>
								<input type="email" bind:value={form.org_email} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.org_email ? 'border-red-500' : 'border-gray-300')} />
							</div>
							<div>
								<label class="block text-sm font-medium">{labelWithRequired('Organization Phone','org_phone')}</label>
								<input type="tel" bind:value={form.org_phone} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.org_phone ? 'border-red-500' : 'border-gray-300')} />
							</div>
						</div>
					{/if}
					...
					{#if editStep === 1}
						<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
							<div class="md:col-span-2">
								...
								<input type="text" bind:value={form.org_address} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.org_address ? 'border-red-500' : 'border-gray-300')} />
							</div>
							<div class="md:col-span-2">
								<label class="block text-sm font-medium">{labelWithRequired('Street 2','org_address2')}</label>
								<input type="text" bind:value={form.org_address2} class="mt-1 w-full border rounded-md px-3 py-2 border-gray-300" />
							</div>
							<div>
								<label class="block text-sm font-medium">{labelWithRequired('City','org_city')}</label>
								<input type="text" bind:value={form.org_city} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.org_city ? 'border-red-500' : 'border-gray-300')} />
							</div>
							<div>
								<label class="block text-sm font-medium">{labelWithRequired('State','org_state')}</label>
								<input type="text" maxlength="2" bind:value={form.org_state} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.org_state ? 'border-red-500' : 'border-gray-300')} />
							</div>
							<div>
								<label class="block text-sm font-medium">{labelWithRequired('Zip Code','org_zip_code')}</label>
								<input type="text" bind:value={form.org_zip_code} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.org_zip_code ? 'border-red-500' : 'border-gray-300')} />
							</div>
						</div>
					{/if}
					...
					{#if editStep === 2}
						<div class="space-y-4">
							<div>
										...
										<tbody>
											{#each DAYS as d}
												<tr class="border-t">
													...
													<td class="px-3 py-2">
														<select class="border rounded px-2 py-1" bind:value={whEdit[d.code].start} disabled={!whEdit[d.code].open}>
															{#each HOUR_OPTS as h}<option value={h}>{h}:00</option>{/each}
														</select>
													</td>
													<td class="px-3 py-2">
														<select class="border rounded px-2 py-1" bind:value={whEdit[d.code].end} disabled={!whEdit[d.code].open}>
															{#each HOUR_OPTS as h}<option value={h}>{h}:00</option>{/each}
														</select>
													</td>
												</tr>
								...
								{#if fieldErrors.working_hours}<p class="text-xs text-red-600 mt-1">{fieldErrors.working_hours}</p>{/if}
							</div>

							<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div>
									<label class="block text-sm font-medium">{labelWithRequired('Days Off','days_off')}</label>
									<input type="text" placeholder="MM/DD, MM/DD" bind:value={form.days_off} class="mt-1 w-full border rounded-md px-3 py-2 border-gray-300" />
									<p class="text-xs text-gray-500 mt-1">Will render like: {formatDaysOff(form.days_off).join(', ')}</p>
								</div>
								<div>
									<label class="block text-sm font-medium">{labelWithRequired('Rides Phone','rides_phone_number')}</label>
									<input type="tel" bind:value={form.rides_phone_number} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.rides_phone_number ? 'border-red-500' : 'border-gray-300')} />
								</div>
								<div>
									<label class="block text-sm font-medium">{labelWithRequired('Client Minimum Age','client_min_age')}</label>
									<input type="number" bind:value={form.client_min_age} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.client_min_age ? 'border-red-500' : 'border-gray-300')} />
								</div>
								<div>
									<label class="block text-sm font-medium">{labelWithRequired('Min Days in Advance','min_days_in_advance_for_ride_requests')}</label>
									<input type="number" bind:value={form.min_days_in_advance_for_ride_requests} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.min_days_in_advance_for_ride_requests ? 'border-red-500' : 'border-gray-300')} />
								</div>
							</div>
						</div>
					{/if}
					...
					{#if editStep === 5}
						<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
							...
							<div>
								...
								<input type="text" bind:value={form.user_initial_password} class={"mt-1 w-full border rounded-md px-3 py-2 " + (fieldErrors.user_initial_password ? 'border-red-500' : 'border-gray-300')} />
							</div>
						</div>
					{/if}
					...
					{#if editStep === 6}
						<div class="text-sm text-gray-700 space-y-2">
							<p class="font-medium">Review your entries. Use Back to make changes.</p>
					...
					<div class="flex justify-between pt-2">
						<button type="button" on:click={editStep === 0 ? closeEdit : prevStep} class="px-4 py-2 border border-gray-300 rounded-md">
							{editStep === 0 ? 'Cancel' : 'Back'}
						</button>
						{#if editStep < STEP_LABELS.length - 1}
							<button type="button" on:click={nextStep} class="px-4 py-2 bg-blue-600 text-white rounded-md">Next</button>
						{:else}
							<button type="submit" disabled={isSaving} class="px-4 py-2 bg-blue-600 text-white rounded-md inline-flex items-center gap-2 disabled:opacity-60">
								<Save class="w-4 h-4" /><span>{isSaving ? 'Saving…' : 'Save Changes'}</span>
							</button>
						{/if}
					</div>
				</form>
